$(document).ready(function () {
    ...
    function mostrarVideoPorAno(ano) {
        $('p[data-ano]').hide();
        $('p[data-ano="' + ano + '"]').fadeIn();
    }

    
    var anoInicial = $("select[id*=ddlAnoFiltro]").val();
    mostrarVideoPorAno(anoInicial);

    
    $("select[id*=ddlAnoFiltro]").on("change", function () {
        var anoSelecionado = $(this).val();
        efetuarFiltroPorAno(anoSelecionado); 
        mostrarVideoPorAno(anoSelecionado);  
        $('.anoList').text($(this).find("option:selected").text());
